//  Create virtual warehouse for data scientist & DBA
CREATE WAREHOUSE DS_WH
WITH WAREHOUSE_SIZE='SMALL'
WAREHOUSE_TYPE='STANDARD'
AUTO_SUSPEND=300
AUTO_RESUME=TRUE
MIN_CLUSTER_COUNT=1
MAX_CLUSTER_COUNT=1
SCALING_POLICY='STANDARD';

CREATE WAREHOUSE DBA_WH
WITH WAREHOUSE_SIZE='XSMALL'
WAREHOUSE_TYPE='STANDARD'
WAREHOUSE_TYPE=300
AUTO_SUSPEND=300
AUTO_RESUME=TRUE
MIN_CLUSTER_COUNT=1
MAX_CLUSTER_COUNT=1
SCALING_POLICY='STANDARD';

//GRANT WAREHOUSE TO ROLES
CREATE ROLE DATA_SCIENTIST;
GRANT USAGE ON WAREHOUSE DS_WH TO ROLE DATA_SCIENTIST;

CREATE ROLE DBA;
GRANT USAGE ON WAREHOUSE  DBA_WH TO ROLE DBA

//ROLES, ACC, AND GRANTS
CREATE USER DS1 PASSWORD='123' LOGIN_NAME='DS1' DEFAULT_ROLE='DATA_SCIENTIST'
DEAFULT_WAREHOUSE='DS_WH' MUST_CHANGE_PASSWORD=FALSE;
GRANT ROLE DATA_SCIENTIST TO USER DS1

CREATE USER DS1 PASSWORD='123' LOGIN_NAME='DS2' DEFAULT_ROLE='DATA_SCIENTIST'
DEAFULT_WAREHOUSE='DS_WH' MUST_CHANGE_PASSWORD=FALSE;
GRANT ROLE DATA_SCIENTIST TO USER DS2

CREATE USER DS1 PASSWORD='123' LOGIN_NAME='DS3' DEFAULT_ROLE='DATA_SCIENTIST'
DEAFULT_WAREHOUSE='DS_WH' MUST_CHANGE_PASSWORD=FALSE;
GRANT ROLE DATA_SCIENTIST TO USER DS3

// DBA ROLE, ACC, AND GRANTS
CREATE USER DBA1 PASSWORD = 'DBA1' LOGIN_NAME = 'DBA1' DEFAULT_ROLE='DBA' DEFAULT_WAREHOUSE = 'DBA_WH'  MUST_CHANGE_PASSWORD = FALSE;
CREATE USER DBA2 PASSWORD = 'DBA2' LOGIN_NAME = 'DBA2' DEFAULT_ROLE='DBA' DEFAULT_WAREHOUSE = 'DBA_WH'  MUST_CHANGE_PASSWORD = FALSE;

GRANT ROLE DBA TO USER DBA1;
GRANT ROLE DBA TO USER DBA2;

// Drop objects again
DROP USER DBA1;
DROP USER DBA2;

DROP USER DS1;
DROP USER DS2;
DROP USER DS3;

DROP ROLE DATA_SCIENTIST;
DROP ROLE DBA;

DROP WAREHOUSE DS_WH;
DROP WAREHOUSE DBA_WH;